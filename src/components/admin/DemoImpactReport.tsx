...
import { Award, BadgeCheck, BookOpen, Building2, CheckCircle2, ChevronRight, Heart, HeartHandshake, Lightbulb, Search, Sparkles, Target, TrendingUp, User, UserCheck, Users, X, Zap } from "lucide-react";
...
import { demoAssessmentSummary, getDemoParticipantAssessment } from "@/lib/assessment-methodology";
...
export function DemoImpactReport() {
  ...
  const participantIndex = DEMO_PARTICIPANTS.findIndex((p) => p.id === participant.id);
  const assessment = getDemoParticipantAssessment(participantIndex);
    ...
    <div className="space-y-7">
      ...
      {viewMode === "group" ? (
        <div className="space-y-7">
          ...
          {/* NEW METHODOLOGY V1 — additive, existing approved report remains intact */}
          <section className="rounded-lg border border-[#EAE5D9] bg-white p-6 shadow-[0_8px_30px_-25px_rgba(15,30,61,0.45)]">
            <div className="flex flex-col justify-between gap-3 border-b border-slate-100 pb-4 sm:flex-row sm:items-end">
              <div><p className="text-[10px] font-extrabold uppercase tracking-widest text-[#C79A3C]">OUTCOME, EXECUTION & VALIDATION</p><h3 className="mt-1 text-base font-extrabold text-[#0F1E3D]">Integrated Transformation Evidence</h3><p className="mt-1 text-xs text-slate-500">Membedakan hasil perubahan, pelaksanaan action plan, partisipasi program, dukungan sosial, dan kekuatan validasi.</p></div>
              <span className="w-fit rounded-full bg-amber-50 px-3 py-1 text-[10px] font-bold text-amber-800 ring-1 ring-amber-200">Methodology v1 · 6 detailed profiles</span>
            </div>
            <div className="mt-5 grid gap-px overflow-hidden rounded-xl bg-slate-200 ring-1 ring-slate-200 sm:grid-cols-2 xl:grid-cols-5">
              {[
                [Target, "Outcome Attainment", demoAssessmentSummary.outcome, "Target 90 hari"],
                [TrendingUp, "Behavior Execution", demoAssessmentSummary.execution, "Action plan & habit"],
                [CheckCircle2, "Program Engagement", demoAssessmentSummary.engagement, "Proses SLJ"],
                [HeartHandshake, "Peer Support", demoAssessmentSummary.peerSupport, "Sahabat Safar"],
                [BadgeCheck, "Validated Outcome", demoAssessmentSummary.validatedOutcome, "Participant + coach"],
              ].map(([Icon, label, value, detail]: any) => <div key={label} className="bg-white p-4"><Icon className="h-4 w-4 text-[#C79A3C]" /><p className="mt-4 text-[10px] font-bold uppercase tracking-wide text-slate-400">{label}</p><strong className="mt-1 block text-3xl font-black text-[#0F1E3D]">{value}<span className="text-sm text-slate-400">/100</span></strong><p className="mt-1 text-[10px] text-slate-500">{detail}</p></div>)}
            </div>
            <div className="mt-4 grid gap-3 sm:grid-cols-3"><div className="bg-[#FAF8F4] p-4"><p className="text-[10px] font-bold text-slate-400">INDICATOR COVERAGE</p><p className="mt-1 text-xl font-black text-[#0F1E3D]">{demoAssessmentSummary.indicatorCoverage}%</p><p className="text-[10px] text-slate-500">18 dari 24 slot indikator aktif</p></div><div className="bg-[#FAF8F4] p-4"><p className="text-[10px] font-bold text-slate-400">COACH VALIDATION</p><p className="mt-1 text-xl font-black text-[#0F1E3D]">{demoAssessmentSummary.coachValidationCoverage}%</p><p className="text-[10px] text-slate-500">6 dari 6 profil detail ditinjau</p></div><div className="bg-[#FAF8F4] p-4"><p className="text-[10px] font-bold text-slate-400">RECIPROCAL SUPPORT</p><p className="mt-1 text-xl font-black text-[#0F1E3D]">{demoAssessmentSummary.reciprocalSupport}%</p><p className="text-[10px] text-slate-500">5 dari 6 profil dukungan dua arah</p></div></div>
            <p className="mt-4 text-[10px] leading-relaxed text-slate-400">Methodology metrics are calculated from the six detailed demo participant records currently available. The approved 25-participant executive dataset above remains unchanged and is not being represented as the denominator for these new fields.</p>
          </section>
        ...
        <div className="space-y-7">
          ...
          <section className="rounded-3xl border border-[#EAE5D9] bg-white p-6"><div className="flex flex-col justify-between gap-2 border-b border-slate-100 pb-4 sm:flex-row sm:items-end"><div><p className="text-[10px] font-extrabold uppercase tracking-widest text-[#C79A3C]">ASSESSMENT EVIDENCE</p><h3 className="mt-1 font-bold text-[#0F1E3D]">Outcome, Execution & Coach Validation</h3></div><span className="w-fit rounded-full bg-emerald-50 px-3 py-1 text-[10px] font-bold text-emerald-700">{assessment.validationStatus}</span></div><div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{[["Participant Outcome", assessment.participantOutcome], ["Behavior Execution", assessment.execution], ["Coach Assessment", assessment.coachAssessment], ["Validated Outcome", assessment.validatedOutcome]].map(([label, value]) => <div key={String(label)} className="bg-[#FAF8F4] p-4"><p className="text-[10px] font-bold text-slate-400">{label}</p><p className="mt-2 text-2xl font-black text-[#0F1E3D]">{value}<span className="text-xs text-slate-400">/100</span></p></div>)}</div><div className="mt-4 grid gap-3 sm:grid-cols-2"><div className="border border-slate-200 p-4"><p className="text-[10px] font-bold text-slate-400">INDICATOR COVERAGE</p><p className="mt-1 text-sm font-bold text-[#0F1E3D]">{assessment.coverage}% · {assessment.activeIndicators} dari 4 indikator aktif</p></div><div className="border border-slate-200 p-4"><p className="text-[10px] font-bold text-slate-400">EVIDENCE STRENGTH</p><p className="mt-1 text-sm font-bold text-[#0F1E3D]">{assessment.evidenceStrength}</p></div></div><p className="mt-4 text-[11px] leading-relaxed text-slate-500">Validated Outcome menggunakan bobot Participant 60% dan Coach 40%. Behavior Execution berasal dari rata-rata completion tiga habit yang sudah tersedia pada mock participant. Outcome dan coach assessment adalah skenario demo metodologi, bukan data produksi.</p></section>
