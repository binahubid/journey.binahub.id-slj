...
const SECTIONS = [
  ...
  { num: 4, title: "Target 90 Hari & Indikator", subtitle: "1 target utama & 3 indikator terukur" },
...
// ─── Area List ───────────────────────────────────────────────────
const AREA_LIST = [
  { id: "Spiritual Growth", icon: Compass, label: "Spiritual Growth", desc: "Kedisiplinan ibadah & kedekatan dengan Allah", sel: "border-amber-400 bg-amber-50 text-amber-800", base: "border-slate-200 bg-white text-slate-700" },
  { id: "Personal Development", icon: Zap, label: "Personal Development", desc: "Integritas, kesabaran & kontrol emosi", sel: "border-blue-400 bg-blue-50 text-blue-800", base: "border-slate-200 bg-white text-slate-700" },
  { id: "Leadership Excellence", icon: Award, label: "Leadership Excellence", desc: "Etos kerja & keteladanan dalam karir", sel: "border-purple-400 bg-purple-50 text-purple-800", base: "border-slate-200 bg-white text-slate-700" },
  { id: "Family Bonding", icon: Users, label: "Family Bonding", desc: "Keharmonisan & komunikasi keluarga", sel: "border-rose-400 bg-rose-50 text-rose-800", base: "border-slate-200 bg-white text-slate-700" },
  { id: "Community Impact", icon: Globe, label: "Community Impact", desc: "Kebermanfaatan sosial & dakwah", sel: "border-teal-400 bg-teal-50 text-teal-800", base: "border-slate-200 bg-white text-slate-700" },
  { id: "Health & Wellbeing", icon: Activity, label: "Health & Wellbeing", desc: "Kesehatan fisik & mental yang optimal", sel: "border-green-400 bg-green-50 text-green-800", base: "border-slate-200 bg-white text-slate-700" },
];
...
export default function JourneyPage() {
  ...
  const targetAlasanRef = useRef("");
  const indicatorsRef = useRef<string[]>(["", "", ""]);
  const actionPlansRef = useRef<{ id?: string; title: string; frequency: string }[]>([]);
    ...
    async function loadData() {
      try {
        ...
        if (journey) {
          ...
          if (Array.isArray(journey.area_transformasi) && journey.area_transformasi.length > 0) setSelectedAreas(journey.area_transformasi);

          // Target: try parse as JSON
          try {
            const td = JSON.parse(journey.main_target || "{}");
            setMainTarget(typeof td === "object" ? (td.target || "") : journey.main_target || "");
            setTargetAlasan(typeof td === "object" ? (td.alasan || "") : "");
          } catch { setMainTarget(journey.main_target || ""); }

          if (Array.isArray(journey.success_indicators)) {
            setIndicators([journey.success_indicators[0] || "", journey.success_indicators[1] || "", journey.success_indicators[2] || ""]);
          }

          // Action Plans
          const { data: plans } = await supabase.from("action_plans").select("id, title, frequency").eq("journey_id", journey.id);
          if (plans && plans.length > 0) {
            setActionPlans(plans);
          } else {
            const { data: uh } = await supabase.from("habits").select("id, title, frequency").eq("user_id", user.id).eq("is_archived", false);
            if (uh && uh.length > 0) setActionPlans(uh);
          }

          // Support Team
          try {
            const { data: team } = await supabase.from("support_team").select("coach_name, sahabat_safar_name").eq("journey_id", journey.id).maybeSingle();
  ...
  useEffect(() => { mainTargetRef.current = mainTarget; }, [mainTarget]);
  useEffect(() => { targetAlasanRef.current = targetAlasan; }, [targetAlasan]);
    ...
    try {
      ...
      switch (sectionNum) {
        ...
        case 3:
          ...
          break;
        case 4:
          await supabase.from("journeys").update({ main_target: JSON.stringify({ target: mainTargetRef.current, alasan: targetAlasanRef.current }), success_indicators: indicatorsRef.current.filter(i => i.trim() !== ""), updated_at: new Date().toISOString() }).eq("id", _journeyId);
          break;
        case 5: {
          ...
          for (const ap of _plans) {
            const { data: apData } = await supabase.from("action_plans").insert({ journey_id: _journeyId, user_id: user.id, title: ap.title, category: "general", frequency: ap.frequency }).select().maybeSingle();
            ...
            if (!existing) await supabase.from("habits").insert({ user_id: user.id, action_plan_id: apData?.id || null, title: ap.title, category: "general", frequency: ap.frequency, source: "action_plan", effective_from: todayStr, is_archived: false });
          }
          const activeTitles = _plans.map(a => a.title);
          const { data: oldHabits } = await supabase.from("habits").select("id, title").eq("user_id", user.id).eq("is_archived", false);
          if (oldHabits) { for (const oh of oldHabits) { if (!activeTitles.includes(oh.title)) await supabase.from("habits").update({ is_archived: true, effective_until: todayStr }).eq("id", oh.id); } }
          break;
        }
        case 6:
          await supabase.from("support_team").upsert({ journey_id: _journeyId, user_id: user.id, coach_name: coachNameRef.current, sahabat_safar_name: sahabatSafarRef.current });
          break;
      }
      setSaveStatus("saved");
  ...
  const isSectionComplete = (num: number) => {
    switch (num) {
      ...
      case 4: return mainTarget.trim().length > 0 && indicators.some(i => i.trim() !== "");
  ...
  const addActionPlan = () => {
    ...
    const next = [...actionPlans, { id: String(Date.now()), title: newActionTitle.trim(), frequency: newActionFreq }];
    setActionPlans(next);
    setNewActionTitle("");
  ...
  const renderSectionContent = (sectionNum: number) => {
    ...
    switch (sectionNum) {
              ...
              {AREA_LIST.map(area => {
                ...
                return (
                  <button key={area.id} onClick={() => toggleArea(area.id)} disabled={locked || (!isSelected && selectedAreas.length >= 3)}
                    ...
                    <div className={`h-9 w-9 rounded-lg flex items-center justify-center shrink-0 ${isSelected ? "bg-white/60" : "bg-slate-100"}`}>
                      <Icon className="h-4 w-4" />
                    </div>
                    <div className="min-w-0">
                      <span className="font-bold text-sm block">{area.label}</span>
                      <span className="text-xs opacity-70 leading-tight block mt-0.5">{area.desc}</span>
                    </div>
                    {isSelected && (
                      <div className="absolute top-2 right-2 h-5 w-5 bg-current/20 rounded-full flex items-center justify-center">
      ...
      case 4:
        return (
          <div className="space-y-5">
            <div>
              <p className="text-sm font-bold text-slate-700 mb-1.5">Target Utama 90 Hari</p>
              <Textarea disabled={locked} value={mainTarget} onChange={e => { setMainTarget(e.target.value); scheduleAutosave(4); }} placeholder="Tuliskan 1 target utama yang ingin Anda capai dalam 90 hari..." className="min-h-[100px] text-sm resize-none border-warm-border focus:border-amber-400 rounded-xl" maxLength={500} />
              <div className="text-right text-xs text-slate-400 mt-1">{mainTarget.length} / 500</div>
            </div>
            <div>
              <p className="text-sm font-bold text-slate-700 mb-1.5">Mengapa target ini penting?</p>
              <Textarea disabled={locked} value={targetAlasan} onChange={e => { setTargetAlasan(e.target.value); scheduleAutosave(4); }} placeholder="Apa motivasi terdalam Anda mencapai target ini?" className="min-h-[80px] text-sm resize-none border-warm-border focus:border-amber-400 rounded-xl" maxLength={300} />
            </div>
            <div>
              <div className="flex items-center gap-2 mb-3 relative">
                <p className="text-sm font-bold text-slate-700">3 Indikator Keberhasilan Terukur</p>
                <button onMouseEnter={() => setShowIndicatorTooltip(true)} onMouseLeave={() => setShowIndicatorTooltip(false)} onClick={() => setShowIndicatorTooltip(v => !v)}
                  className="h-5 w-5 rounded-full bg-slate-100 text-slate-400 hover:bg-amber-100 hover:text-amber-600 flex items-center justify-center transition-colors shrink-0">
                  <Info className="h-3 w-3" />
                </button>
                {showIndicatorTooltip && (
                  <div className="absolute left-0 top-7 z-50 bg-navy-900 text-white text-xs rounded-xl p-3 shadow-xl w-56">
                    <p className="font-bold mb-1">Indikator SMART</p>
                    <p className="opacity-80">Specific · Measurable · Achievable · Relevant · Time-bound</p>
                    <p className="mt-2 opacity-60 text-[10px]">Contoh: "Sholat 5 waktu berjamaah selama 60 dari 90 hari"</p>
                  </div>
                )}
              </div>
              <div className="space-y-2.5">
                {indicators.map((ind, idx) => (
                  <div key={idx} className="flex items-center gap-3">
                    <div className="h-7 w-7 rounded-full bg-amber-100 text-amber-700 text-xs font-bold flex items-center justify-center shrink-0">{idx + 1}</div>
                    <Input disabled={locked} value={ind} onChange={e => { const next = [...indicators]; next[idx] = e.target.value; setIndicators(next); scheduleAutosave(4); }} placeholder={`Indikator ${idx + 1}...`} className="flex-1 text-sm border-warm-border focus:border-amber-400 rounded-xl h-10" />
                  </div>
                ))}
              </div>
            </div>
      ...
      case 5:
        return (
          <div className="space-y-4">
            <p className="text-sm text-slate-600">Kebiasaan yang akan Anda jalankan</p>
            {!locked && (
            ...
            {showAddHabit && !locked && (
              <div className="border border-amber-200 bg-amber-50 rounded-xl p-3 space-y-2">
                <Input value={newActionTitle} onChange={e => setNewActionTitle(e.target.value)} onKeyDown={e => e.key === "Enter" && addActionPlan()} placeholder="Nama kebiasaan baru..." className="text-sm border-amber-200 focus:border-amber-400 rounded-lg h-10 bg-white" autoFocus />
                <div className="flex gap-2">
                  <select value={newActionFreq} onChange={e => setNewActionFreq(e.target.value)} className="flex-1 border border-amber-200 rounded-lg px-3 text-sm text-slate-600 bg-white focus:border-amber-400 focus:outline-none h-10">
                    <option>Harian</option>
                    <option>2x Seminggu</option>
                    <option>3x Seminggu</option>
                    <option>Mingguan</option>
                  </select>
                  <Button onClick={addActionPlan} className="bg-amber-500 hover:bg-amber-600 text-white rounded-lg h-10 px-4 font-bold">Tambah</Button>
                  <Button variant="outline" onClick={() => { setShowAddHabit(false); setNewActionTitle(""); }} className="rounded-lg h-10 px-3 border-amber-200">
                    <X className="h-4 w-4" />
                  </Button>
                </div>
              </div>
            ...
            <div className="space-y-2">
              {actionPlans.length === 0 && (
                <div className="text-center py-8 text-slate-400 text-sm border-2 border-dashed border-slate-200 rounded-xl">
                  Belum ada kebiasaan. Tambahkan kebiasaan pertama Anda!
                </div>
              )}
              {actionPlans.map((ap, idx) => {
                const { bg, iconColor, Icon } = getHabitStyle(ap.title);
                return (
                  <div key={idx} className="flex items-center gap-3 p-3 bg-white border border-warm-border rounded-xl shadow-2xs hover:border-amber-200 transition-colors group">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-400">{ap.frequency} · Target: 90 Hari</p>
                    </div>
                    {!locked && (
            ...
            {actionPlans.length > 0 && (
              <div className="bg-navy-900/5 border border-navy-900/10 rounded-xl p-4">
                <p className="text-xs font-bold text-navy-900 mb-0.5">Preview di Dashboard Harian</p>
                <p className="text-xs text-slate-400 mb-3">Hari ke-{dayCount} · {actionPlans.length} Kebiasaan</p>
                <div className="flex flex-wrap gap-2">
                  {actionPlans.slice(0, 6).map((ap, i) => {
                    const { bg, iconColor, Icon } = getHabitStyle(ap.title);
                    return <div key={i} className={`h-8 w-8 rounded-lg ${bg} ${iconColor} flex items-center justify-center`} title={ap.title}><Icon className="h-4 w-4" /></div>;
                  })}
                  {actionPlans.length > 6 && <div className="h-8 w-8 rounded-lg bg-slate-100 text-slate-500 flex items-center justify-center text-xs font-bold">+{actionPlans.length - 6}</div>}
                </div>
              </div>
            )}
          </div>
      ...
      case 6:
        return (
          <div className="space-y-4">
            <div className="bg-amber-50 border-l-4 border-amber-400 rounded-r-xl p-4">
              <div className="flex items-center gap-2 mb-1"><Users className="h-4 w-4 text-amber-600" /><span className="text-sm font-bold text-amber-800">Apa itu Sahabat Safar?</span></div>
              <p className="text-sm text-amber-900">Sahabat Safar adalah teman akuntabilitas Anda selama 90 hari — sosok yang akan saling mengingatkan dan mendukung perjalanan transformasi ini.</p>
            </div>
            <div>
              <p className="text-sm font-bold text-slate-700 mb-1.5">Nama Sahabat Safar</p>
              <Input disabled={locked} value={sahabatSafar} onChange={e => { setSahabatSafar(e.target.value); scheduleAutosave(6); }} placeholder="Nama teman akuntabilitas Anda..." className="text-sm border-warm-border focus:border-amber-400 rounded-xl h-11" />
            </div>
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-1"><span className="text-slate-400 text-sm">👨‍🏫</span><span className="text-xs font-bold text-slate-600">Coach Anda</span></div>
              <p className="text-sm font-semibold text-navy-900">{coachName}</p>
              <p className="text-xs text-slate-400 mt-0.5">Ditunjuk oleh Admin BinaJourney</p>
            </div>
          </div>
        );
    }
